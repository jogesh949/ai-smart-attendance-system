...
const ClassPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/admin/classes', { ...formData, department_id: Number(formData.department_id) });
      toast.success('Class added successfully', { id: tId });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Class deleted successfully', { id: tId });
      fetchData();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage institutional classes.</p>
        </div>
        <Button onClick={() => setIsModalOpen(true)}><Plus className="w-4 h-4 mr-2" /> Add Class</Button>
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={Layers} title="No Classes Found" message="There are no classes registered yet." />
        ) : (
